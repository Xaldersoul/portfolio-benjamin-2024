...
export default function NavBar({
  ...
  return (
    <nav aria-label="Main navigation">
      <ul className="flex flex-col justify-between rounded-b-lg bg-slate-950 px-4 py-2 md:m-4 md:flex-row md:items-center md:rounded-xl">
        ...
        <div
          className={clsx(
            "fixed bottom-0 left-0 right-0 top-0 z-50 flex flex-col items-end gap-4 bg-slate-950 pr-4 pt-14 transition-transform duration-300 ease-in-out md:hidden",
...
function NameLogo({ name }: { name: KeyTextField }) {
  return (
    <Link
      ...
      className="text-xl font-extrabold tracking-tighter text-slate-50 transition-colors hover:text-blue-400"
    >
      {name}
    </Link>
  );
        ...
        <React.Fragment key={label}>
          <li>
            ...
            >
              <span
                className={clsx(
                  "absolute inset-0 z-0 h-full rounded bg-blue-400 transition-transform  duration-300 ease-in-out group-hover:translate-y-0",
                  pathname.includes(asLink(link) as string)
                    ? "translate-y-6"
                    : "translate-y-8"
                )}
